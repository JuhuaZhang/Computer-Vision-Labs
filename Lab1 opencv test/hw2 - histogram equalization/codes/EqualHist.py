...
if __name__ == '__main__':
    ...
    # 读取原始图像
    img = cv2.imread('./GrayImg.jpg', cv2.IMREAD_GRAYSCALE)
    ...
    plt.subplot(121), plt.imshow(img, cmap=plt.cm.gray), plt.title(
        'origin_image', fontdict={'weight': 'normal', 'size': 20}), plt.axis('off')  # 坐标轴关闭
    plt.subplot(122), plt.imshow(new_img, cmap=plt.cm.gray), plt.title(
        'new_image', fontdict={'weight': 'normal', 'size': 20}), plt.axis('off')  # 坐标轴关闭
